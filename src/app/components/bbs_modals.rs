...
pub struct BBSModalProps {}

#[function_component]
pub fn BBSModal(props: &BBSModalProps) -> Html {
    let BBSModalProps {} = props;
    let (_, bbs_modal_dispatch) = use_store::<BBSModalState>();

    let onclick = {
        let bbs_modal_dispatch = bbs_modal_dispatch.clone();
        Callback::from(move |_| {
            bbs_modal_dispatch.reduce(|_| BBSModalState(false).into());
        })
    };
            ...
            <div class="fixed inset-0 z-[960] overflow-y-auto">
                <div class="flex items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-[90vw] text-light-text">
                        ...
                        <div class="w-[90vw] h-[85vh] overflow-y-auto">
                            <img src="/public/masakari.webp"  alt="" />
                        </div>
                    </div>
                </div>
            </div>
