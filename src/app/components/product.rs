...
pub(crate) fn Product(props: &ProductProps) -> Html {
    ...
    html! {
        ...
        class={classes!("w-fit", "h-fit", "m-8", "flex", "max-w-[512px]", "hover:border-2", "hover:border-picture-frame", classes)}>
            <figure class="h-fit relative">
                <img src={product_info.img_src.clone()} onload={onload} alt={product_info.title.clone()} width=512 />
                if new {
                    <svg class={classes!("absolute", "-top-[32px]", "-left-[32px]", "bg-picture-frame", "rounded-full",
                                "bg-gradient-to-r", "from-[#6080B0]", "via-[#08DCF9]", "to-[#FF2775]")}
                    height="64px" width="64px" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" space="preserve" fill="#000000">
